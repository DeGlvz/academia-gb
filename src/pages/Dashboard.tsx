      ...
      <main className="flex-1 container px-4 py-8 max-w-5xl space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid w-full max-w-3xl grid-cols-5">
            <TabsTrigger value="inicio" className="gap-2"><Home className="h-4 w-4" />Inicio</TabsTrigger>
            <TabsTrigger value="clases" className="gap-2"><BookOpen className="h-4 w-4" />Mis clases</TabsTrigger>
            <TabsTrigger value="gratis" className="gap-2"><Gift className="h-4 w-4" />Clases gratis</TabsTrigger>
            <TabsTrigger value="lecturas" className="gap-2"><FileText className="h-4 w-4" />Mi rincón de lectura</TabsTrigger>
            <TabsTrigger value="perfil" className="gap-2"><Settings className="h-4 w-4" />Mi perfil</TabsTrigger>
          </TabsList>
                    ...
                    <Card key={post.id} className={`hover:shadow-md transition-shadow ${isRead ? "border-green-200 bg-green-50/30" : ""}`}>
                      <CardContent className="p-4 flex items-center justify-between">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            {isRead ? <CheckCircle className="h-4 w-4 text-green-600 shrink-0" /> : <Circle className="h-4 w-4 text-muted-foreground shrink-0" />}
                            <p className={`text-sm font-medium truncate ${isRead ? "text-muted-foreground" : "text-foreground"}`}>{post.title}</p>
                          </div>
                          <p className="text-xs text-muted-foreground mt-1 ml-6">{new Date(post.created_at).toLocaleDateString("es-MX")} · {post.read_time || 5} min lectura</p>
                        </div>
                        <Button size="sm" variant="ghost" asChild className="shrink-0">
                          <Link to={`/blog/${post.slug}`}>{isRead ? "Leer otra vez" : "Comenzar a leer"}</Link>
                        </Button>
                      </CardContent>
                    </Card>
                  );
