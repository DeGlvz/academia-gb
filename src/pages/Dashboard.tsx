    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 w-full px-4 py-6 md:py-8">
        <div className="w-full max-w-7xl mx-auto space-y-6 md:space-y-8">
          {/* Header del perfil */}
          <motion.div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-5 text-center sm:text-left" initial="hidden" animate="visible" variants={fadeInUp}>
            <div className="h-20 w-20 sm:h-20 sm:w-20 rounded-full bg-primary/10 flex items-center justify-center shrink-0 overflow-hidden border-2 border-primary/30">
              {profileData.avatar ? (
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">Miembro desde {profileData.registeredAt}</p>
            </div>
          </motion.div>
          ...
          {/* Stats cards */}
          <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4" initial="hidden" animate="visible" variants={staggerContainer}>
            {[
              ...
              <motion.div key={stat.label} variants={staggerItem}>
                <Card>
                  <CardContent className="pt-4 pb-3 md:pt-5 md:pb-4 flex flex-col items-center text-center gap-2">
                    {stat.icon}
                    <span className="text-xl md:text-2xl font-bold text-foreground">{stat.value}</span>
                    <span className="text-[10px] md:text-xs text-muted-foreground leading-tight">{stat.label}</span>
                  </CardContent>
                </Card>
              </motion.div>
            ))}
          ...
          <Tabs defaultValue="clases" className="space-y-4">
            <TabsList className="grid w-full max-w-md grid-cols-2 mx-auto sm:mx-0">
              <TabsTrigger value="clases" className="gap-2">📚 Mis clases</TabsTrigger>
              <TabsTrigger value="blog" className="gap-2">📖 Mi blog</TabsTrigger>
            </TabsList>

            {/* Tab: Mis Clases */}
            <TabsContent value="clases" className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <h2 className="text-lg md:text-xl font-display font-bold text-foreground">Mis clases</h2>
            ...
            <TabsContent value="blog" className="space-y-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <h2 className="text-lg md:text-xl font-display font-bold text-foreground">Mi blog</h2>
                <Button variant="ghost" size="sm" asChild className="gap-1 font-body text-xs">
          ...
          <motion.section className="space-y-4" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={staggerContainer}>
            <h2 className="text-lg md:text-xl font-display font-bold text-foreground">Mis herramientas</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <CardContent className="pt-6 flex flex-col sm:flex-row items-start gap-4">
                    ...
                    <div className="flex-1 space-y-1">
                      ...
                      {hasCalculadoraAccess ? (
                        <Button size="sm" variant="outline" className="mt-2 gap-1 font-body text-xs" asChild>
