  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      try {
        ...
        // Si no requiere clase específica
        if (!config.required_class_id) {
          setHasAccess(true);
          return;
        }

        // Verificar si el usuario tiene la clase requerida
        const { data: enrollment, error: enrollError } = await supabase
          .from("enrolled_classes")
          .select("id")
          .eq("user_id", user.id)
          .eq("class_id", config.required_class_id)
          .maybeSingle();
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container px-4 py-8 max-w-4xl">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-display font-bold text-foreground">Calculadora Panadero Pro</h1>
            ...
            <Card>
              <CardContent className="pt-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="flour" className="text-sm font-medium">Harina (g)</Label>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="hidden sm:grid grid-cols-12 gap-2 text-xs font-medium text-muted-foreground px-1">
                  ...
                  <span className="col-span-2 text-center">Total (g)</span>
                ...
                <div className="flex flex-wrap sm:grid sm:grid-cols-12 gap-2 items-center bg-primary/5 rounded-lg px-3 py-2.5">
                  ...
                  <span className="sm:col-span-2 text-center text-sm font-semibold">{flourWeight}g</span>
                  <span className="sm:col-span-2 text-center text-sm text-muted-foreground">{pieces > 1 && <>{Math.round((flourWeight / pieces) * 10) / 10}g/pieza</>}</span>
                ...
                {computed.map((ing, idx) => (
                  <div key={idx} className="flex flex-wrap sm:grid sm:grid-cols-12 gap-2 items-center px-3 py-2 rounded-lg hover:bg-muted/50 transition-colors border sm:border-0">
                    ...
                    <span className="text-sm font-medium sm:col-span-2 sm:text-center">
                      <span className="sm:hidden text-muted-foreground text-xs">= </span>{ing.totalGrams}g
                    </span>
                    <span className="text-sm text-muted-foreground sm:col-span-2 sm:text-center">
                      {pieces > 1 ? `${ing.perPiece}g/pza` : ""}
                    </span>
                ...
                <div className="flex flex-wrap sm:grid sm:grid-cols-12 gap-2 items-center px-3 py-2.5 bg-secondary rounded-lg">
                  ...
                  <span className="hidden sm:block sm:col-span-2" />
                  <span className="text-sm font-bold text-foreground sm:col-span-2 sm:text-center">{totalWeight}g</span>
                  <span className="text-sm font-semibold text-muted-foreground sm:col-span-2 sm:text-center">{pieces > 1 ? `${Math.round((totalWeight / pieces) * 10) / 10}g/pza` : ""}</span>
                  <span className="hidden sm:block sm:col-span-2" />
                </div>
              </CardContent>
            </Card>
