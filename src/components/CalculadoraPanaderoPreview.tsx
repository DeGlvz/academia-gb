...
import { Calculator, Save, Download, Trash2, Plus, Scale } from "lucide-react";
...
interface CalculadoraPanaderoPreviewProps {
  config: {
...
const STORAGE_KEY = "gaby-baker-formulas-preview";
...
const CalculadoraPanaderoPreview = ({ config }: CalculadoraPanaderoPreviewProps) => {
  const { toast } = useToast();
  
  const [flourWeight, setFlourWeight] = useState(1000);
  const [pieces, setPieces] = useState(1);
  const [ingredients, setIngredients] = useState<Ingredient[]>(config.default_ingredients);
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    min={1}
                    value={flourWeight}
                    onChange={(e) => setFlourWeight(Number(e.target.value) || 0)}
...
export default CalculadoraPanaderoPreview;
